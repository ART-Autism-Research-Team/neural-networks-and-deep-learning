...
def basic_conv(n=3, epochs=60):
    for j in range(n):
        ...
        net = Network([
            ...
            SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
        net.SGD(
          training_data, epochs, mini_batch_size, 0.1, validation_data,
          test_data)
    return net
        ...
        for j in range(3):
            ...
            net = Network([
                ...
                FullyConnectedLayer(n_in=40 * 4 * 4, n_out=100,
                                    activation_fn=ReLU),
                SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
            net.SGD(training_data, 60, mini_batch_size, 0.03, validation_data,
                    test_data, lmbda=lmbda)
...
def expanded_data(n=100):
    ...
    for j in range(3):
        print "Training with expanded data, %s neurons in the FC layer, run num %s" % (
            n, j)
        net = Network([
            ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28),
                          filter_shape=(20, 1, 5, 5),
...
def plot_errors(error_locations, erroneous_predictions=None):
    ...
    fig = plt.figure()
    error_images = [np.array(test_x[i]).reshape(28, -1) for i in
                    error_locations]
